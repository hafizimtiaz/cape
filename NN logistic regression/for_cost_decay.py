...
def load_dataset(N = 10000, D = 50, dist = 0.25):
    """
...
train_X, train_Y, test_X, test_Y = load_dataset(5000, 5)
...
maxIter = 200
batch_size = 64
acc_nonp_tr = np.zeros([nensemble, ])
acc_nonp_ts = np.zeros([nensemble, ])
acc_conv_tr = np.zeros([nensemble, len(epsilon_all)])
acc_conv_ts = np.zeros([nensemble, len(epsilon_all)])
acc_cape_tr = np.zeros([nensemble, len(epsilon_all)])
acc_cape_ts = np.zeros([nensemble, len(epsilon_all)])
costs_nonp = np.zeros([nensemble, np.int(maxIter)])
costs_conv = np.zeros([nensemble, np.int(maxIter)])
costs_cape = np.zeros([nensemble, np.int(maxIter)])
...
for rep in range(nensemble):
    ...
    acc_nonp_tr[rep] = acc_tr * 100
    acc_nonp_ts[rep] = acc_ts * 100
...
#        pdb.set_trace()
        ...
        acc_cape_tr[rep, eps_id] = acc_tr * 100
        acc_cape_ts[rep, eps_id] = acc_ts * 100
        if eps_id == 0:
        ...
        acc_conv_tr[rep, eps_id] = acc_tr * 100
        acc_conv_ts[rep, eps_id] = acc_ts * 100
        if eps_id == 0:
            costs_conv[rep, :] = costs_conv_tmp

#%% save results
res_filename = 'synth_cost_decay_D50'
np.savez(res_filename, acc_nonp_tr, acc_nonp_ts, acc_conv_tr, acc_conv_ts, acc_cape_tr, acc_cape_ts, epsilon_all, delta, costs_nonp, costs_conv, costs_cape)
